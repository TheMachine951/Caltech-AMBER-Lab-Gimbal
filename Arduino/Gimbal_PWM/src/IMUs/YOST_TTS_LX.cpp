#include "YOST_TTS_LX.h"
	...
	int32_t YOST_TTS_LX::getFusionedData(void)
	{
		...
		//Rotate quat
		setOffset(fusionedDataTemp.quat);
	...
	void YOST_TTS_LX::setAxisConvention(void)
	{
		...
		const std::vector<uint8_t> data{35};
		sendMessage(cmd,data);                 
	}
	...
	void YOST_TTS_LX::setOffset(float quat[])
	{
		float n0 = quat[0]*config_.quadOffset[0] 
		- quat[1]*config_.quadOffset[1] 
		- quat[2]*config_.quadOffset[2]
		- quat[3]*config_.quadOffset[3];

		float n1 = quat[0]*config_.quadOffset[1] 
		+ quat[1]*config_.quadOffset[0] 
		- quat[2]*config_.quadOffset[3]
		+ quat[3]*config_.quadOffset[2];

		float n2 = quat[0]*config_.quadOffset[2] 
		+ quat[1]*config_.quadOffset[3] 
		+ quat[2]*config_.quadOffset[0]
		- quat[3]*config_.quadOffset[1];

		float n3 = quat[0]*config_.quadOffset[3] 
		- quat[1]*config_.quadOffset[2]
		+ quat[2]*config_.quadOffset[1]
		+ quat[3]*config_.quadOffset[0];
